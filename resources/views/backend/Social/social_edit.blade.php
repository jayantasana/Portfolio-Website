        ...
        <div class="col-xl-8 m-auto">
          <div class="form-layout form-layout-4">
            ...
            <form action="{{ route('SocialUpdate') }}" method="POST">
                @csrf
            <div class="row">
                <input type="hidden" name="socialhidden_id" class="form-control" value="{{ $socialedit->id }}">
                <label class="col-sm-4 form-control-label">Add Social Name: <span class="tx-danger">*</span></label>
                <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                    <input type="text" name="socialname" class="form-control" value="{{ $socialedit->socialname }}">
                </div>
            </div><!-- row -->
        </div>
    ...
    <script>
        ...
        @if(session('SocialUpdate'))
            Swal.fire({
                position: 'center',
                icon: 'success',
                title: 'Social Update Successfully',
                showConfirmButton: false,
                timer: 1000
            })
        @endif
